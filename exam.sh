...
add_users() {
    ...
    while read -r user; do
        if id "$user" &>/dev/null; then
            ...
            sudo useradd -m "$user" && echo -e "${GREEN}User $user created ok${NC}" || echo -e "${RED}Fail to create $user${NC}"
        fi
    done < "$file"
}
...
setup_projects() {
    ...
    if [[ -z $user || -z $num ]]; then
        echo -e "${RED}missing args, give username and project count${NC}"
        exit 1
    fi
...
sys_report() {
    out=$1
    if [[ -z $out ]]; then
        echo -e "${RED}please give output file name${NC}"
        exit 1
    fi
